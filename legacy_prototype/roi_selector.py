...
class ROISelector:
    ...
    def __init__(self):
        ...
        self.canvas.create_text(
            ...
            60,
            text="【拖拽框选】按住鼠标左键拖拽选出腾讯会议中 iPad 题目的显示区域\n框选完成后松开鼠标，按 Enter 键确认，按 Esc 取消",
            fill="yellow",
...
def get_roi_interactive(force_reselect=False):
    """
    获取监控区域：
    如果有历史配置且不强制重新选择，优先询问是否复用；
    否则拉起图形化选择器。
    """
    saved = load_saved_roi()
    if saved and not force_reselect:
        print(f"\n[检测到历史监控区域配置] x={saved['x']}, y={saved['y']}, 宽={saved['width']}, 高={saved['height']}")
...
if __name__ == "__main__":
    roi = get_roi_interactive(force_reselect=True)
    print("最终选择的 ROI:", roi)
